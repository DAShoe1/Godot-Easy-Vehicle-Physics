...
func _process(delta):
    ...
    var front_axle := vehicle.global_transform * vehicle.FrontAxlePosition
    var rear_axle := vehicle.global_transform * vehicle.RearAxlePosition
    ...
    if debug_sets[current_debug_set] == "Inputs" or debug_sets[current_debug_set] == "All":
        debug_ui.draw_debug_line("front_steer", front_axle, vehicle.global_transform.basis * Vector3(-sin(vehicle.SteeringInput * vehicle.MaxSteeringAngle), 0.0, -cos(vehicle.SteeringInput * vehicle.MaxSteeringAngle)), Color.GRAY)
        debug_ui.draw_debug_line("front_real_steer", front_axle, vehicle.global_transform.basis * Vector3(-sin(vehicle.TrueSteeringAmount), 0.0, -cos(vehicle.TrueSteeringAmount)), Color.WHITE)
        ...
        debug_ui.draw_debug_line("throttle", center_of_gravity + (vehicle.global_transform.basis.x * 1.5), vehicle.global_transform.basis.y * vehicle.ThrottleAmount, Color.GREEN, false, Vector2(0.0, 0.0))
        ...
        debug_ui.draw_debug_line("torque", center_of_gravity + (vehicle.global_transform.basis.x * 1.5), vehicle.global_transform.basis.y * (vehicle.TorqueOutput / vehicle.MaxTorque), Color.AQUA, false, Vector2(2.0, 0.0))
        ...
        debug_ui.draw_debug_line("dt", center_of_gravity + (vehicle.global_transform.basis.x * 1.5), vehicle.global_transform.basis.y * (vehicle.ClutchTorque / vehicle.MaxTorque), Color.DEEP_SKY_BLUE, false, Vector2(4.0, 0.0))
        ...
        debug_ui.draw_debug_line("brake", center_of_gravity + (vehicle.global_transform.basis.x * 1.5), vehicle.global_transform.basis.y * (vehicle.BrakeAmount), Color.RED, false, Vector2(6.0, 0.0))
        ...
        debug_ui.draw_debug_line("clutch", center_of_gravity + (vehicle.global_transform.basis.x * 1.5), vehicle.global_transform.basis.y * (vehicle.ClutchAmount), Color.YELLOW, false, Vector2(8.0, 0.0))
        ...
        debug_ui.draw_debug_line("hb", center_of_gravity + (vehicle.global_transform.basis.x * 1.5), vehicle.global_transform.basis.y * (vehicle.HandbrakeInput), Color.ORANGE, false, Vector2(10.0, 0.0))
    
    if debug_sets[current_debug_set] == "Drivetrain" or debug_sets[current_debug_set] == "All":
        debug_ui.draw_debug_line("front_torque", center_axle, (front_axle - center_axle) * vehicle.TrueTorqueSplit , Color.ORANGE)
        debug_ui.draw_debug_line("rear_torque", center_axle, (rear_axle - center_axle) * (1.0 - vehicle.TrueTorqueSplit) , Color.ORANGE)
        debug_ui.draw_debug_line("fl_split", front_axle, (vehicle.FrontLeftWheel.global_position - front_axle) * ((vehicle.FrontAxle.AppliedSplit + 1.0) * 0.5) * vehicle.TrueTorqueSplit, Color.ORANGE)
        debug_ui.draw_debug_line("fr_split", front_axle, (vehicle.FrontRightWheel.global_position - front_axle) * (1.0 - ((vehicle.FrontAxle.AppliedSplit + 1.0) * 0.5)) * vehicle.TrueTorqueSplit, Color.ORANGE)
        debug_ui.draw_debug_line("rl_split", rear_axle, (vehicle.RearLeftWheel.global_position - rear_axle) * ((vehicle.RearAxle.AppliedSplit + 1.0) * 0.5) * (1.0 - vehicle.TrueTorqueSplit), Color.ORANGE)
        debug_ui.draw_debug_line("rr_split", rear_axle, (vehicle.RearRightWheel.global_position - rear_axle) * (1.0 - ((vehicle.RearAxle.AppliedSplit + 1.0) * 0.5)) * (1.0 - vehicle.TrueTorqueSplit), Color.ORANGE)
    
    if debug_sets[current_debug_set] == "Stability" or debug_sets[current_debug_set] == "All":
        var normalized_vector := vehicle.StabilityTorqueVector.normalized();
        debug_ui.draw_debug_line("upright", center_of_gravity, Vector3(normalized_vector.z, 0.0, normalized_vector.x) * vehicle.StabilityTorqueVector.length() * 0.001, Color.MAGENTA)
        debug_ui.draw_debug_line("yaw_front", front_axle, (vehicle.global_transform.basis.x * vehicle.StabilityYawTorque * 0.001) / vehicle.StabilityYawStrength, Color.RED)
        debug_ui.draw_debug_line("yaw_rear", rear_axle, (-vehicle.global_transform.basis.x * vehicle.StabilityYawTorque * 0.001) / vehicle.StabilityYawStrength, Color.RED)
    
    for wheel : GEVPWheel in vehicle.WheelArray:
        var wheel_color := Color.GREEN
        if not wheel.LimitSpin:
        ...
        if debug_sets[current_debug_set] == "Suspension Forces" or debug_sets[current_debug_set] == "All":
            debug_ui.draw_debug_line(wheel.name + "spring_force", wheel.global_position, wheel.global_transform.basis.y * wheel.SpringForce * 0.0002, Color.RED)
            debug_ui.draw_debug_line(wheel.name + "arb_force", wheel.global_position, wheel.global_transform.basis.y * (wheel.AntirollForce + wheel.DampingForce) * 0.0002, Color.BLUE)
            debug_ui.draw_debug_line(wheel.name + "damp_force", wheel.global_position, wheel.global_transform.basis.y * wheel.DampingForce * 0.0002, Color.YELLOW)
            ...
            debug_ui.draw_debug_line(wheel.name + "spring_length", wheel.global_position, -wheel.global_transform.basis.y * (wheel.SpringCurrentLength / wheel.SpringLength), Color.GREEN, false, Vector2(4 * signf(wheel.position.x), 0))
            
        
        if debug_sets[current_debug_set] == "Tire Forces" or debug_sets[current_debug_set] == "All":
            var wheel_force : Vector3 = ((wheel.global_transform.basis.x * wheel.ForceVector.x) + (wheel.global_transform.basis.z * wheel.ForceVector.y)) * 0.0002
            var wheel_slip : Vector3 = ((wheel.global_transform.basis.x * -wheel.SlipVector.x) + (wheel.global_transform.basis.z * -wheel.SlipVector.y)) * 2.0
            debug_ui.draw_debug_line(wheel.name + "force", wheel.LastCollisionPoint, wheel_force, Color.RED)
            debug_ui.draw_debug_line(wheel.name + "slip", wheel.LastCollisionPoint, wheel_slip, Color.YELLOW)
